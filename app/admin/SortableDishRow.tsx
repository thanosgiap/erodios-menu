    ...
    <tr ref={setNodeRef} style={style}>
      ...
      <td style={{ padding: '10px 14px' }}>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={() => onEdit(dish)} style={{ background: 'transparent', border: '0.5px solid rgba(122,106,85,0.4)', borderRadius: '2px', padding: '4px 10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit', color: '#7A6A55' }}>Edit</button>
          <button onClick={() => onToggle(dish)} style={{ background: 'transparent', border: `0.5px solid ${dish.visible ? 'rgba(60,140,80,0.4)' : 'rgba(122,106,85,0.4)'}`, borderRadius: '2px', padding: '4px 10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit', color: dish.visible ? '#2D7A45' : '#7A6A55' }}>{dish.visible ? 'Hide' : 'Show'}</button>
          <button onClick={() => onDelete(dish.id)} style={{ background: 'transparent', border: '0.5px solid rgba(180,80,60,0.4)', borderRadius: '2px', padding: '4px 10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit', color: '#9B3A2A' }}>Delete</button>
        </div>
      </td>
    </tr>
  )
